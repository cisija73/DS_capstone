...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        filtered_df.rename(columns={'class': 'Successful launches (Total)'}, inplace=True)
        fig = px.pie(filtered_df, 
                     values='Successful launches (Total)',
                     names='Launch Site',
                     title='Total Success Launches By Site')
    else:
        filtered_df = spacex_df[spacex_df['Launch Site']==entered_site]
        filtered_df = filtered_df['class'].value_counts().to_frame().reset_index()
        filtered_df.rename(columns={'index': 'class', 'class': 'count'}, inplace=True)
        fig = px.pie(filtered_df, 
                     values='count',
                     names='class',
...
def get_scatter_chart(entered_site, selected_payload):
    ...
    filtered_df = filtered_df[filtered_df['Payload Mass (kg)']<=selected_payload[1]]
    if entered_site == 'ALL':
